...
class TestModelManager(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._models = ModelManager(self._client)

    def test_len(self) -> None:
        self._add_some_models()
        self.assertEqual(len(self._models), len(self._client.models))

    def test_contains(self) -> None:
        self._add_some_models()
        for model in self._client.models:
            self.assertIn(model.id, self._models)

    def test_iter(self) -> None:
        self._add_some_models()
        self.assertEqual(
            [model.id for model in self._models],
            [model.id for model in self._client.models]
        )

    def test_get(self) -> None:
        ref = self._client.add_model()
        model = self._models.get(ref.id)
    ...
    def test_remove(self) -> None:
        ...
        self._models.remove(to_remove)
        self.assertEqual(len(self._models), count - 1)
        self.assertNotIn(to_remove, self._models)
