...
class TestCameraPathHandler(unittest.TestCase):
    ...
    def test_camera_path(self):
        ...
        self.assertEqual(camera_path.get_nb_frames(), 41)
        try:
            camera_path.get_key_frame(100)
        except Exception as e:
            self.assertIsInstance(e, KeyError)

    def test_get_orbit_around_target(self):
        model = {
            'bounding_box': False,
            'bounds': {
                'max': [1.0, 1.0, 1.0],
        ...
        camera_key_frames = CameraPathHandler.get_orbit_around_target(model, 2)
        self.assertEqual(
            camera_key_frames,
            [
                [
                ...
                [
                    [-1.0, 0.5, 0.5000000000000002],
                    [1.0, 0.0, -1.4802973661668753e-16],
                    [0.0, 1.0, 0.0]
                ]
            ]
        )
